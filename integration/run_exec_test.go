...
func TestIntegration(t *testing.T) {
	...
	t.Run("docker compose up", func(t *testing.T) {
		...
		for _, file := range []string{localExecFilename, execFilename, runFilename} {
			...
			expectedMin := expectedExecutions
			if file == runFilename {
				// Run jobs can be one execution short when docker compose stops quickly
				// and the last run is still in progress.
				expectedMin = expectedExecutions - 1
			}

			if count < expectedMin {
				t.Errorf("expected at least %d lines in %s, but got %d. File content:\n%s", expectedMin, file, count, content)
			}
			...
			for resultFile, resultCount := range resultFiles {
				expectedMin := expectedExecutions
				if file == runFilename {
					expectedMin = expectedExecutions - 1
				}
				...
				if resultCount < expectedMin {
					t.Errorf("expected at least %d job execution files for %s, but got %d", expectedMin, resultFile, resultCount)
				}
			}
		}
	})
