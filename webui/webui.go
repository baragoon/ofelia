...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .job-row{grid-column:1/-1;background:var(--card-bg);border:1px solid var(--card-border);border-left:3px solid #9ca3af;border-radius:6px;padding:.75rem 1rem;display:grid;grid-template-columns:auto 1fr auto;column-gap:.8rem;align-items:center}
    @supports (grid-template-columns: subgrid){.job-row{grid-template-columns:subgrid;column-gap:normal}}
