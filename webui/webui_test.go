...
// testLogger implements core.Logger with no-op methods for testing
...
// setDockerHandler sets the unexported dockerHandler field in cli.Config using reflection
func setDockerHandler(cfg *cli.Config, handler *cli.DockerHandler) {
	v := reflect.ValueOf(cfg).Elem().FieldByName("dockerHandler")
	reflect.NewAt(v.Type(), unsafe.Pointer(v.UnsafeAddr())).Elem().Set(reflect.ValueOf(handler))
}

// setDockerClients sets the unexported dockerClients field in cli.DockerHandler using reflection
func setDockerClients(handler *cli.DockerHandler, clients map[string]*docker.Client) {
	v := reflect.ValueOf(handler).Elem().FieldByName("dockerClients")
	reflect.NewAt(v.Type(), unsafe.Pointer(v.UnsafeAddr())).Elem().Set(reflect.ValueOf(clients))
}
...
func TestBuildWebUIStateGroupsJobsByHost(t *testing.T) {
	config := cli.NewConfig(&testLogger{})
	handler := &cli.DockerHandler{}
	clients := map[string]*docker.Client{"docker-a": nil, "docker-b": nil}
	setDockerClients(handler, clients)
	setDockerHandler(config, handler)

	config.ExecJobs["docker-a::job-a"] = &cli.ExecJobConfig{ExecJob: core.ExecJob{BareJob: core.BareJob{Schedule: "@every 1m", Command: "echo a"}, Container: "container-a"}}
	config.RunJobs["job-b"] = &cli.RunJobConfig{RunJob: core.RunJob{BareJob: core.BareJob{Schedule: "@every 2m", Command: "echo b"}, Image: "alpine"}, DockerHost: "docker-b"}
	config.LocalJobs["local-job"] = &cli.LocalJobConfig{LocalJob: core.LocalJob{BareJob: core.BareJob{Schedule: "@every 3m", Command: "date"}, Dir: "/tmp"}}
...
func TestBuildWebUIStateLastRunFields(t *testing.T) {
	...
	job := &cli.ExecJobConfig{ExecJob: core.ExecJob{BareJob: core.BareJob{Schedule: "@every 1m", Command: "echo hello"}, Container: "c1"}}
...
func TestBuildWebUIStateCommandMasking(t *testing.T) {
	...
	config.ExecJobs["secret-job"] = &cli.ExecJobConfig{
